...
fn main() {
    ...
    let (tx, rx): (channel::Sender<i32>, channel::Receiver<i32>) = channel::unbounded();
    let rx1 = rx.clone();
    let rx2 = rx.clone();

    for n in 1..=10 {
        println!("Sending value {} to channel...", n);
        tx.send(n).unwrap();
    }
    ...
    let handle_rx1 = thread::spawn(move || {
        for val in rx1 {
            println!("Received value {} via rx1...", val);
        }
    });

    let handle_rx2 = thread::spawn(move || {
        for val in rx2 {
            println!("Received value {} via rx2...", val);
        }
    });
